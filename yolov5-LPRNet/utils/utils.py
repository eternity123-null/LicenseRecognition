...
from utils.LPRload_data import CHARS 
...
def findContoursAndDrawBoundingBox(image_rgb):
    ...
    # print(line_lower)
    # print(line_upper)
    [vx1, vy1, x1, y1] = cv2.fitLine(line_lower,cv2.DIST_L2,0,0.01, 0.01)
    [vx2, vy2, x2, y2]=cv2.fitLine(line_upper,cv2.DIST_L2,0,0.01, 0.01)
    mean_k=((vy1/vx1)+(vy2/vx2))/2
    print("k:",mean_k)
    height, width = rgb.shape[:2]  # 图片的高度和宽度
    x0, y0 = width//2, height//2
    angle = np.rad2deg(np.arctan(mean_k))
    print("angle:",angle[0])
    MAR = cv2.getRotationMatrix2D((x0,y0), angle[0], 1.0)
    image = cv2.warpAffine(rgb, MAR, (width, height))
...
def return_angle(img):
    ...
    line_lower = np.array(line_lower)
    line_upper = np.array(line_upper)
    rgb = cv2.copyMakeBorder(img,30,30,0,0,cv2.BORDER_REPLICATE)
    ...
    if len(line_lower) > 0:
        ...
        bA = line_lowerA[3] - kA*line_lowerA[2]
    else:
        kA = 0
    if len(line_upper) > 0:
        ...
        bB = line_upperB[3] - kB*line_upperB[2]
    else:
        kB = 0
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            for j, a in enumerate(d):  # per item
                ...
                # cutout1 = im0[i][int(a[1]-height*0.23):int(a[3]+height*0.23), int(a[0]-width*0.14):int(a[2]+width*0.14)]
                # # cutout = im0[i][int(a[1]):int(a[3]), int(a[0]):int(a[2])]
                # cv2.imshow("plate",cutout)
                # cv2.waitKey(0)
                # angle = return_angle(cutout1)
                
                # height, width = cutout.shape[:2]  # 图片的高度和宽度
                # x0, y0 = width//2, height//2
                
                # print("angle:",angle)
                # MAR = cv2.getRotationMatrix2D((x0,y0), int(angle), 1.0)
                # cutout = cv2.warpAffine(cutout, MAR, (width, height))
                # cutout, best_angle = fast_deskew.deskew_image(cutout)
                # cv2.imshow("plate",cutout)
                # cv2.waitKey(0)
                # cutout=findContoursAndDrawBoundingBox(cutout)
                # cv2.imshow("deskew",cutout)
                # cv2.waitKey(0)
...
def change_cv2_draw(image,strs,local,sizes,colour):
    ...
    font = ImageFont.truetype("simsun.ttc", 50, encoding="unic")
