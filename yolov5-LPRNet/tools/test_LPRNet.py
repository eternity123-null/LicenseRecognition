...
from utils.LPRload_data import CHARS, CHARS_DICT, LPRDataLoader
...
def get_parser():
    ...
    parser.add_argument('--test_img_dirs',
                        default=r"D:\zcd\CV\rec_imgs\test", help='the test images path')
    parser.add_argument('--dropout_rate', default=0, help='dropout rate.')
    parser.add_argument('--lpr_max_len', default=8,
                        help='license plate number max length.')
    parser.add_argument('--test_batch_size', default=100,
                        help='testing batch size.')
    parser.add_argument('--phase_train', default=False,
                        type=bool, help='train or test phase flag.')
    parser.add_argument('--num_workers', default=8, type=int,
                        help='Number of workers used in dataloading')
    parser.add_argument('--cuda', default=True, type=bool,
                        help='Use cuda to train model')
    parser.add_argument('--show', default=F, type=bool,
                        help='show test image and its predict result or not.')
    parser.add_argument(
        '--pretrained_model', default='./weights/LPRNet__iteration_11000.pth', help='pretrained base model')
...
def Greedy_Decode_Eval(Net, datasets, args):
    ...
    batch_iterator = iter(DataLoader(datasets, args.test_batch_size,
    ...
    t2 = time.time()
    print("[Info] Test Speed: {}s 1/{}]".format((t2 - t1) /
          len(datasets), len(datasets)))
