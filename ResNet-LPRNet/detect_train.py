    ...
    def train(self):
        ...
        for epoch in range(config.epoch):
            ...
            for i, (images, labels) in enumerate(self.data_loader):
                ...
                self.optimizer.step()
                if i % 10 == 0:
                    print("epoch:",epoch+100,"i:", i,"loss:", loss.item(), 'loss_c:', loss_c.item(), 'loss_p:', loss_p.item())
                if i % 100 == 0:
                    torch.save(self.net.state_dict(), 'newweight/wpod_net.pt')
                loss_sum += loss.item()
            logs = f'epoch:{epoch+100},loss:{loss_sum / len(self.data_loader)}'
            print(logs)
            torch.save(self.net.state_dict(), 'newweight/wpod_net.pt')
